...
--[[
    These functions test if the iteration/step needs collecting a specific information
    ...
    This speeds up a little the run time of argos
]]
...
function need_closest_obstacle()
    return (STATE == READY or STATE == EXPLORING or STATE == WALK_AWAY or STATE == WAIT or STATE == WALK_AWAY_NESTING or CPT.global.current < CPT.global.reach_light)
end
...
function need_light()
    return (STATE == GRABING or STATE == WALK_AWAY or STATE == WAIT or STATE == WALK_AWAY_NESTING or STATE == FINISHING or CPT.global.current < CPT.global.reach_light)
end
...
-- DISTANCE PARAMETERS
DISTANCE_AVOID_ROBOT = 75

-- LIGHT
LIGHT_VALUE_DROP = 0.95
LIGHT_VALUE_FINISHING = 0.9

-- Counters : most counters have a current value and a limit value. The idea is to increment the counter where the robot is doing something and when the counter is higher than its limit value, we do something. Some counters have several limit values, each one for a different usage.
-- Example : For global, the robots try first to reach light first, then start job at a precise iteration.
-- A lot of cpt limits are expressed using other variables like speed. This is because of the manual tunning. To avoid perturbating to much the parameters, I identified relations between parameters and if the speed evolves, the counters adapt.
CPT = {
    grabing          = { current = 0, max_for_hunter = math.floor(5000/SPEED) , max_for_nester = math.floor(10000/SPEED) },
    walk_away        = { current = 0, max = math.floor(3600/SPEED_WALK_AWAY) },
    global           = { current = 0, reach_light = 100, start_job = 300 },
    steping_in_cache = { current = 0, max = math.floor(225/(SPEED*SLOWDOWN.walking_in_cache)) },
    leaving_cache    = { current = 0, max = math.floor(600/(SPEED*SLOWDOWN.walking_in_cache)) },
    unloading        = { current = 0, backwards = 1, turn = 1+30, drop = 1+30+1, advance = 1+30+math.floor(500/SPEED) },
    reach_object     = { current = 0, max = math.floor(3000/SPEED) },
    finishing        = { current = 0, to_finishing = 100, to_waiter = 100 } -- set to 0 everytime an object is grabed, incremented every time 
}
...
--[[
    ...
    Each switch changes the corresponding global variable and reset counters when it is necessary.
    In addition, the velocity of the robot is set to (0,0) to ensure a precise control over it's movements.
...
function avoid(angle,slowdown_ratio)
    ...
    local_speed = SPEED*slowdown_ratio*slowdown_near_cache
...
function reach(angle,slowdown_ratio)
    ...
    local_speed = SPEED*slowdown_ratio
...
function walk_foward()
    if STATE == WALK_AWAY or STATE == WALK_AWAY_NESTING then
        robot.wheels.set_velocity(SPEED_WALK_AWAY,SPEED_WALK_AWAY)
    else
...
function walk_backwards(slowdown_ratio)
    ...
    robot.wheels.set_velocity(-SPEED*slowdown_ratio,-SPEED*slowdown_ratio)
end
...
--[[
    The first thing that the controller does is turning on the central led of the robot
    according to it's state and job.

    Then the controller collect all the information needed by calling the "check" functions

    Finally, is the router. One and only one block of instruction is going to be executed.
    This block of instructions depends on the state and job of the robot.

    For example : if JOB == HUNTER and STATE == EXPLORING, the block of instruction
    executed is associated with the hunter exploration phase : walking randomly, avoiding
    other robots and looking for objects.
]]
...
function step()
    ...
    ------------------------------------------ SHUFFLE ROBOTS --------------------------------------------------
    ------------------------------------------------------------------------------------------------------------
    if CPT.global.current < CPT.global.reach_light then -- first try reaching the light, avoid having robots stuck behind the nest
    ...
    ------------------------------------------ START WORKING ---------------------------------------------------
    ------------------------------------------------------------------------------------------------------------
    elseif CPT.global.current == CPT.global.start_job then
        start_job()
    ------------------------------------------------------------------------------------------------------------
    ---------------------------------------- HUNTER CONTROLLER -------------------------------------------------
    ------------------------------------------------------------------------------------------------------------
    elseif JOB == HUNTER then
        ...
        -- This block can be executed with another block. It is only verifying the job of the robot.
        ...
        if STATE == READY then
            if CPT.global.current >= CPT.global.start_job then
                start_job()
            elseif GREY.sensed then
                avoid_grey()
            elseif OBSTACLE.sensed then
                avoid(OBSTACLE.angle)
            elseif (ROBOT.sensed) and (ROBOT.distance < DISTANCE_AVOID_ROBOT/3) then
                avoid(ROBOT.angle)
            else
                walk_foward()
            end
        ...
        elseif STATE == EXPLORING then
            ...
            elseif not(OBJECT.sensed) then -- not object detected, just avoid every thing
                if OBSTACLE.sensed then
                    avoid(OBSTACLE.angle)
                elseif (ROBOT.sensed) and (ROBOT.distance < DISTANCE_AVOID_ROBOT) then
                    avoid(ROBOT.angle)
                else
                    walk_foward()
                end
            elseif not(IS_TOUCHING_OBJECT_WITH_GRIPPER) then -- object detected, try to reach it, avoiding the robots
                if OCCUPIED_ROBOT.sensed and OCCUPIED_ROBOT.distance < DISTANCE_AVOID_ROBOT*2 then
        ...
        elseif STATE == GRABING then
            ...
            elseif LIGHT.sensed then -- there is light ! follow it !
                if ROBOT.sensed and ROBOT.distance < DISTANCE_AVOID_ROBOT/2 then
    ...
    elseif JOB == NESTER then
        ...
        elseif STATE == WAIT then
            ...
            reach_with_gripper(0)
            if CPT.finishing.current > CPT.finishing.to_finishing then
            ...
            elseif not(OBJECT.sensed) then -- not object detected, just avoid every thing and reach light
                if OBSTACLE.sensed then
                    avoid(OBSTACLE.angle)
                elseif (OCCUPIED_ROBOT.sensed) and (OCCUPIED_ROBOT.distance < DISTANCE_AVOID_ROBOT*2) then
                    avoid(OCCUPIED_ROBOT.angle)
                elseif (ROBOT.sensed) and (ROBOT.distance < DISTANCE_AVOID_ROBOT) then
                    avoid(ROBOT.angle)
                elseif LIGHT.sensed then
        ...
        elseif STATE == FINISHING then
            CPT.finishing.current = CPT.finishing.current + 1
            if CPT.finishing.current > CPT.finishing.to_waiter then
                switch_state_wait()
            elseif GREY.sensed then
                avoid_grey()
            elseif LIGHT.sensed and LIGHT.value > LIGHT_VALUE_FINISHING then
                avoid(LIGHT.angle)
            else
                walk_foward()
            end
